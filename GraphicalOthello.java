...
import java.lang. *;
/**
 * Single players graphical version of Othello with 
...
public class GraphicalOthello extends Othello implements ActionListener 
{
    ...
    ///This loop is reused from the otheollo loop given with some changes to take onto account the GUI 
    public void loop() {
            int status = ONGOING;
            Move move = null;
               ...
               if (!(generateMoves().isEmpty())) {
                   do {///This program doesnt like to be in a loop with notihng in it so
                       ...
                       if(nextPlayerMove != null)
                       {
                           move = nextPlayerMove;
                           nextPlayerMove = null;
                           if(canPlay(move) == false){System.out.println("Cant go there");}
                        }
                    }
